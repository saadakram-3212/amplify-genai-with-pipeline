...
module "load_balancer" {
  ...
  domain_name             = var.domain_name != "" ? "${local.env}-${var.domain_name}" : ""
...
module "cognito_pool" {
  ...
  cognito_domain          = var.cognito_domain != "" ? "${local.env}-${var.cognito_domain}" : ""
  userpool_name           = "${local.env}-${var.userpool_name}"
  provider_name           = "${local.env}-${var.provider_name}"
  sp_metadata_url         = var.sp_metadata_url
  callback_urls           = var.domain_name != "" ? ["https://${local.env}-${var.domain_name}/api/auth/callback/cognito", "http://localhost:3000/api/auth/callback/cognito"] : ["http://localhost:3000/api/auth/callback/cognito"]
  logout_urls             = var.domain_name != "" ? ["https://${local.env}-${var.domain_name}", "http://localhost:3000"] : ["http://localhost:3000"]
...
output "domain_name" {
  value       = var.domain_name != "" ? "${local.env}-${var.domain_name}" : module.load_balancer.alb_dns_name
  description = "The domain name or ALB DNS used for the application"
}
