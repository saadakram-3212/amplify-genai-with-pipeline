...
docker run --rm --network host --entrypoint pip -v $(pwd):/var/task -w /var/task public.ecr.aws/lambda/python:3.11 install --no-cache-dir --upgrade -r requirements.txt -t ./python --platform manylinux2014_x86_64 --only-binary=:all:
